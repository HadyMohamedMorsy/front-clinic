...
export class AuthService {
  ...
  forgetPassword(credentials: ForgetModel) {
    return this.#api.request('post', 'user/forgot-password', credentials);
  }

  resetPassword(credentials: ResetModel) {
    return this.#api
      .request('post', 'user/reset-password', credentials)
      .pipe(tap(({ data }) => this.doLogin(data)));
  }

  verifyPassword(credentials: VerifyModel) {
    return this.#api.request('post', 'user/verify-token-reset-expire', credentials);
  }
