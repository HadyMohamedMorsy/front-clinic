...
export class DatePickerComponent extends FieldType<FieldTypeConfig> {
  ...
  ngOnInit() {
    ...
    if (this.formControl?.value && this.field.model.id) {
      ...
      const date = this.props.showTime
        ? parse(value, 'MMM dd, yyyy | hh:mm a', new Date())
        : new Date(value);
      const formattedDate = this.props.showTime
        ? format(date, 'yyyy-MM-dd HH:mm')
        : format(date, 'yyyy-MM-dd');

      this.formControl?.setValue(date);
      this.field.model[dateKey as string] = new Date(formattedDate);
    }
