...
> {
    ...
    this.dialogRef.onClose
      ...
      .subscribe((record) => {
        ...
        this.records.update((records) =>
          records.map((item) =>
            item._id === record.id ? { ...item, ...record } : item
          )
        );
      });
  }
  ...
  protected deleteRecord(record: R) {
    this.api
      .request('post', this.indexMeta.endpoints.delete, { id: record._id })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(() => {
        this.records.update((records) =>
          records.filter((i) => i._id !== record._id)
        );
        this.totalRecords.update((totalRecords) => totalRecords - 1);
        this.recordsFiltered.update((recordsFiltered) => recordsFiltered - 1);
      });
